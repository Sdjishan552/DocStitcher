...
function showDownloadPicker(title) {
  return new Promise(resolve => {
    _dlResolve = resolve;
    document.getElementById('dlPickerTitle').textContent = title || 'Choose download format';
...
function removeDocument(docId) {
  if (state.docs.length <= 1) { toast('Keep at least one document card.', 'error'); return; }
...
function renderDocumentCard(doc) {
  ...
  const estimateText = doc.estimate
    ...
    : ready
      ...
      : (doc.oneSided ? 'Add front image' : 'Add front and back');
  const typeLabel = doc.oneSided ? 'ONE-SIDED PDF' : 'A4 PDF';
  ...
  return `
    <section class="card document-card" data-doc-id="${doc.id}">
      <div class="document-card-head">
        ...
        <button class="btn-ghost icon-btn" title="Remove" onclick="removeDocument(${doc.id})">×</button>
      </div>
      ...
      <div class="document-settings">
        <div class="ctrl-group">
          ...
          <select class="ctrl-select" id="orientation-${doc.id}" onchange="updateDocOrientation(${doc.id}, this.value)">
            <option value="portrait"${doc.orientation==='portrait'?' selected':''}>Portrait</option>
            <option value="landscape"${doc.orientation==='landscape'?' selected':''}>Landscape</option>
          </select>
        </div>
        <div class="ctrl-group quality-control">
          ...
          <div class="target-size-wrap" style="margin-top:8px">
            <input type="number" class="target-size-input" id="targetKb-${doc.id}" min="20" max="10240" placeholder="Target KB" oninput="updateTargetKbLabel(${doc.id}, this)">
            <span class="ctrl-label" style="white-space:nowrap;align-self:center">KB</span>
            <button class="target-size-btn" onclick="applyTargetKb(${doc.id})">Set</button>
          </div>
        </div>
      ...
      <div class="doc-layout-preview ${doc.orientation}">
        <div class="paper-mini paper-mini-canvas" title="${ready ? 'Click to preview' : 'Upload images to preview'}" onclick="openPreviewLightbox(${doc.id})">
      ...
      <label class="signature-toggle">
        ...
        <span>Add signature image (with keyboard placement)</span>
      </label>
      ...
      <div class="document-actions">
        <button class="btn-primary" onclick="downloadDocument(${doc.id})" ${ready ? '' : 'disabled'}>Download</button>
        ...
        ${doc.signatureEnabled && doc.files.sign ? `<button class="btn-secondary" onclick="openSignaturePlacer(${doc.id})">🎯 Place Signature</button>` : ''}
      </div>
    </section>
  `;
}
...
function renderSignControls(doc) {
  ...
  return `
    <div class="sign-scale-wrap">
      <label class="ctrl-label">Signature size &nbsp;<span id="signScaleLabel-${doc.id}">${label}</span></label>
      ...
      <div class="sign-offset-info" id="signOffset-${doc.id}" style="font-size:11px;color:var(--text-muted);margin-top:4px">
        Position: X=${doc.signOffsetX.toFixed(1)}mm Y=${doc.signOffsetY.toFixed(1)}mm
      ...
      <div class="drop-zone compact-drop" id="${dropId}"
        ...
        ` : `
          <div class="drop-placeholder">
            ...
            <div class="drop-label">${optional ? 'Optional signature' : 'Click or drag here'}</div>
            <div class="drop-hint">${slot==='sign' ? 'JPG, PNG, WebP' : 'JPG, PNG, WebP or PDF'}</div>
          </div>
        `}
      </div>
      <div class="slot-info ${fileObj ? '' : 'empty'}">
        <span class="slot-fname">${fileObj ? escapeHtml(fileObj.file.name) : (optional ? 'No signature' : 'Required')}</span>
...
async function applyTargetKb(docId) {
  ...
  if (isNaN(targetKb)||targetKb<20||targetKb>10240) { kbInput.classList.add('ts-error'); toast('Enter 20–10240 KB.','error'); return; }
  if (!isDocReady(doc)) { toast('Upload images first.','error'); return; }
...
function confirmSigPlacement() {
  if (_sigPlacerDocId !== null) {
    ...
    toast('Signature position saved!');
  }
  closeSigPlacer();
}
...
function handleFile(file, docId, slot) {
  ...
  if (slot === 'sign' && isPDF) { toast('Signature must be an image (JPG/PNG/WebP).','error'); return; }
  if (!isPDF && !isImg) { toast('Please select JPG, PNG, WebP or PDF.','error'); return; }
...
function loadPdfAsImage(file, docId, slot) {
  const doc = getDoc(docId);
  toast('Converting PDF page to image...', 'success');
  ...
  reader.onload = async (e) => {
    try {
      ...
      console.error(err);
      toast('Could not read PDF. Try a JPG/PNG instead.', 'error');
    }
  };
...
function updateEstimateText(doc, fallback) {
  ...
  else if(!isDocReady(doc)) el.textContent=doc.oneSided?'Add front image':'Add front and back';
  else if(doc.estimate) el.textContent=fmtBytes(doc.estimate);
  else el.textContent=doc.estimateStatus==='working'?'Calculating...':'Pending';
}
...
async function downloadDocument(docId) {
  const doc = getDoc(docId);
  if (!isDocReady(doc)) { toast('Upload required images first.','error'); return; }

  const fmt = await showDownloadPicker('Choose download format');
  if (!fmt) return;
  ...
  try {
    ...
    toast('Downloading '+cleanFilename(doc.name)+'.'+fmt);
  } catch(err) {
...
async function downloadAllDocuments() {
  ...
  if(!readyDocs.length){toast('No ready documents.','error');return;}
  const fmt=await showDownloadPicker('Choose format for all downloads');
  if(!fmt)return;
  ...
  try {
    ...
    console.error(err); toast('Download error: '+err.message,'error');
  } finally {
    if(btn)btn.disabled=false;
  }
}
...
function sdtSendToApp(imgElId,filenameBase) {
  ...
  if(!img||!img.src||img.src.startsWith('data:,')) {sdt.toast('Generate output first.','error');return;}
...
function renderSlotPicker() {
  ...
  body.innerHTML=state.docs.map(doc=>{
    ...
    const frontBtn=fo?`<button class="btn-primary slot-occupied" disabled title="Occupied"><span class="slot-occ-icon">✓</span> Front</button>`:`<button class="btn-primary" onclick="doSendToSlot(${doc.id},'front')">Front</button>`;
    const backBtn=doc.oneSided?`<button class="btn-secondary" style="opacity:0.3" disabled title="One-sided">—</button>`
      :(bo?`<button class="btn-primary slot-occupied" disabled><span class="slot-occ-icon">✓</span> Back</button>`:`<button class="btn-primary" onclick="doSendToSlot(${doc.id},'back')">Back</button>`);
    const signBtn=so?`<button class="btn-secondary slot-occupied" disabled><span class="slot-occ-icon">✓</span> Sign</button>`:`<button class="btn-secondary" onclick="doSendToSlot(${doc.id},'sign')">Sign</button>`;
    return `<div class="slot-picker-doc">
      <div class="slot-picker-doc-name">${escapeHtml(doc.name||'document '+doc.number)}${doc.oneSided?' <small>(one-sided)</small>':''}</div>
...
function doSendToSlot(docId,slot) {
  const doc=getDoc(docId); if(!doc)return;
  if(doc.files[slot]){toast(`${SLOT_META[slot].label} is occupied.`,'error');return;}
...
function sendToNewDocument() {
  ...
  toast('Created new document card.');
}
...
window.sdt = (() => {
  ...
  function loadMultiToolFile(file) {
    if(!file||(file.type&&!file.type.startsWith('image/')&&file.type!=='application/pdf'&&!file.name.endsWith('.pdf'))){
      toast('Select an image or PDF.','error'); return;
    }
    // If PDF: convert first
    if(file.type==='application/pdf'||file.name.toLowerCase().endsWith('.pdf')) {
      toast('Converting PDF first...','success');
      const reader=new FileReader();
      reader.onload=async(e)=>{
        try {
          if(!window.pdfjsLib) {
          ...
          img.src=dataUrl;
        } catch(err){toast('Could not convert PDF.','error');}
      };
      reader.readAsArrayBuffer(file);
      return;
    }
  ...
  function drawMtCropCanvas(rotated) {
    ...
    mtDisplayScale=Math.min(1,maxW/rotated.width);
  ...
  function bindMtCropEvents() {
    ...
    cv.onmousedown=e=>{mtCropDragging=true;mtCropStartX=e.offsetX;mtCropStartY=e.offsetY;mtCropDisplayRect=null;renderMultiTool();};
    cv.onmousemove=e=>{
      if(!mtCropDragging)return;
      const x=Math.min(mtCropStartX,e.offsetX),y=Math.min(mtCropStartY,e.offsetY);
      const w=Math.abs(e.offsetX-mtCropStartX),h=Math.abs(e.offsetY-mtCropStartY);
      mtCropDisplayRect={x,y,w,h};
      // Redraw canvas overlay
      const cv2=document.getElementById('mtCropCanvas');
      if(!cv2)return;
      const ctx=cv2.getContext('2d');
      // Just overlay the selection box without full re-render
      if(mtOrigImg) {
        // Full redraw from stored state
        const deg=((mtState.rotateDeg%360)+360)%360,swap=deg===90||deg===270;
        const srcW=mtOrigImg.naturalWidth,srcH=mtOrigImg.naturalHeight,rW=swap?srcH:srcW,rH=swap?srcW:srcH;
        const rotC=document.createElement('canvas');rotC.width=rW;rotC.height=rH;
        const rCtx=rotC.getContext('2d');
        rCtx.save();rCtx.translate(rW/2,rH/2);rCtx.rotate(deg*Math.PI/180);
        rCtx.scale(mtState.flipH?-1:1,mtState.flipV?-1:1);rCtx.drawImage(mtOrigImg,-srcW/2,-srcH/2);rCtx.restore();
        ctx.drawImage(rotC,0,0,cv2.width,cv2.height);
      }
      ctx.strokeStyle='rgba(249,107,63,0.9)';ctx.lineWidth=2;ctx.setLineDash([4,4]);
      ctx.strokeRect(x,y,w,h);ctx.setLineDash([]);
      ctx.fillStyle='rgba(249,107,63,0.08)';ctx.fillRect(x,y,w,h);
    };
    cv.onmouseup=e=>{
      ...
      if(mtCropDisplayRect&&mtCropDisplayRect.w>4&&mtCropDisplayRect.h>4){
        ...
        const srcW=mtOrigImg.naturalWidth,srcH=mtOrigImg.naturalHeight;
        const rW=swap?srcH:srcW,rH=swap?srcW:srcH;
        const scl=Math.min(1,(document.getElementById('mtCropWrap').offsetWidth||680)/rW);
        mtState.cropRect={
          x:Math.round(mtCropDisplayRect.x/scl),
          y:Math.round(mtCropDisplayRect.y/scl),
          w:Math.round(mtCropDisplayRect.w/scl),
          h:Math.round(mtCropDisplayRect.h/scl),
        };
    ...
    // Touch
    cv.addEventListener('touchstart',e=>{const r=cv.getBoundingClientRect();cv.onmousedown({offsetX:e.touches[0].clientX-r.left,offsetY:e.touches[0].clientY-r.top});},{passive:true});
    cv.addEventListener('touchmove',e=>{const r=cv.getBoundingClientRect();cv.onmousemove({offsetX:e.touches[0].clientX-r.left,offsetY:e.touches[0].clientY-r.top});},{passive:true});
    cv.addEventListener('touchend',()=>cv.onmouseup({}),{passive:true});
  }
  ...
  function mtApplyCrop(){renderMultiTool();toast('Crop applied!');document.getElementById('mtApplyCropBtn').classList.add('active-state');}
  ...
  async function mtDownload() {
    ...
    // Show format picker
    const fmt=await showDownloadPicker('Download as...');
    if(!fmt)return;
  ...
  async function downloadWatermarked(){
    const fmt=await showDownloadPicker('Download watermarked image');
    if(!fmt)return;
    const src=document.getElementById('wmOutImg').src;if(!src)return;
    if(fmt==='pdf'){
      if(!window.jspdf||!window.jspdf.jsPDF){toast('PDF lib not loaded','error');return;}
